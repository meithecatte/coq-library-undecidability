...
Section Enumerability.
  ...
  Proof.
    ...
    - rename x into phi. induction 1; try congruence; subst.
      ...
      + destruct IHprv as [m1], (el_T phi) as [m2]. exists (1 + m1 + m2). cbn. in_app 6. in_collect phi; finish.
      + now exists 0.
      + destruct (el_T phi) as [m1], (el_T psi) as [m2]. exists (1 + m1 + m2). cbn. in_app 7. in_collect (phi, psi); finish.
    - intros [m H]; induction m in A, x, H |-*; cbn in *.
      + now apply Ctx.
      + repeat invert_list_in.
        * eauto.
        * eapply II; eauto.
        * eapply IE; eapply IHm; eauto.
        * eapply AllI; eapply IHm; eauto.
        * eapply AllE; eapply IHm; eauto.
        * destruct b; repeat invert_list_in. eapply Exp, IHm; eauto.
        * destruct p; repeat invert_list_in. eapply Pc.
  Qed.
  ...
  Proof.
    ...
    - intros [m]. induction m in x, H0 |-*; cbn in *.
      ...
      + repeat invert_list_in.
        1: now apply IHm. apply IHm in H1. apply (enum_p He) in H0. unfold containsL in *. firstorder congruence.
  Qed.
  ...
  Proof.
    ...
    - intros (A & [m1] % (enum_el A (@enum_containsL _ _ (cumul L) (to_cumul_cumulative L) HL)) & [m2] % (enum_el x (enum_prv A))).
      exists (1 + m1 + m2). cbn. in_app 2. eapply in_concat. eexists. split. in_collect A. all: finish.
    - intros [m]. induction m in x, H |-*; cbn in *. 1: contradiction. repeat invert_list_in. 1: now eapply IHm.
      exists x1; split.
